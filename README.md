...
If you load **Meta Llama 3** checkpoints, use **`transformers>=4.40,<4.46`** with **PyTorch 2.1** (as in `requirements.txt`). Older than 4.40 mis-builds GQA and can fail weight load with `[1024, 4096]` vs `[4096, 4096]`. **Transformers 4.46+** (and **5.x**) assume newer PyTorch APIs (e.g. `torch.utils._pytree.register_pytree_node` or torch ≥ 2.4); on **torch 2.1.x** you may get import errors until you cap `transformers` or upgrade PyTorch to **≥ 2.2** (or **≥ 2.4** for 5.x). Also use **`fschat>=0.2.36`**: older `fschat==0.2.20` declares `transformers<4.29`, which conflicts with Llama 3–compatible versions. This repo registers a **`llama-3`** chat template when the installed FastChat build does not include it.
